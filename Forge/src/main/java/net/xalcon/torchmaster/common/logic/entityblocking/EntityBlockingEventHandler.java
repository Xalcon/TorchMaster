...
public class EntityBlockingEventHandler
{
    ...
    public static void onCheckSpawn(LivingSpawnEvent.CheckSpawn event)
    {
        ...
        if (log) Constants.LOG.debug("CheckSpawn - IsSpawner: {}, Reason: {}, Type: {}, Pos: {}/{}/{}", event.isSpawner(), event.getSpawnReason(), EntityType.getKey(event.getEntity().getType()), event.getX(), event.getY(), event.getZ());
        ...
        world.getCapability(ModCaps.TEB_REGISTRY).ifPresent(reg ->
        {
            if(reg.shouldBlockEntity(entity, pos))
            {
                ...
                if (log) Constants.LOG.debug("Blocking spawn of {}", EntityType.getKey(event.getEntity().getType()));
                //event.getEntity().addTag("torchmaster_removed_spawn");
            }
            else
            {
                if (log) Constants.LOG.debug("Allowed spawn of {}", EntityType.getKey(event.getEntity().getType()));
            }
        });
    }
    ...
    public static void onVillageSiegeEvent(VillageSiegeEvent event)
    {
        ...
        boolean log = TorchmasterConfig.GENERAL.logSpawnChecks.get();
        if (log) Constants.LOG.debug("VillageSiegeEvent - Pos: {}", event.getAttemptedSpawnPos());
        if(!TorchmasterConfig.GENERAL.aggressiveSpawnChecks.get() && event.getResult() == Event.Result.ALLOW) return;
    ...
    {
        boolean log = TorchmasterConfig.GENERAL.logSpawnChecks.get();
        if (log) Constants.LOG.debug("DoSpecialSpawn - Reason: {}, Type: {}, Pos: {}/{}/{}", event.getSpawnReason(), EntityType.getKey(event.getEntity().getType()), event.getX(), event.getY(), event.getZ());
    ...
    public static void onGlobalTick(TickEvent.ServerTickEvent event)
    {
        ...
        {
            if(TorchmasterForge.server == null) return;

            for(ServerLevel level : TorchmasterForge.server.getAllLevels())
            {
